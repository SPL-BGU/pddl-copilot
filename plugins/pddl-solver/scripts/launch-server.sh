...
for i in $(seq 1 $MAX_WAIT); do
    ...
    if [ "$i" -eq 1 ]; then
        echo "[pddl-solver] Waiting for Docker daemon to start..." >&2
    fi
    if [ "$i" -eq $MAX_WAIT ]; then
        echo "[pddl-solver] Docker daemon not responding after ${MAX_WAIT}s. Is Docker Desktop running?" >&2
        exit 1
    fi
    sleep 1
done
...
if docker image inspect "$REGISTRY_IMAGE" &>/dev/null; then
    ...
    echo "[pddl-solver] Pulling pre-built image from GHCR..." >&2
    if docker pull "$REGISTRY_IMAGE" >&2 2>&1; then
        IMAGE="$REGISTRY_IMAGE"
        echo "[pddl-solver] Image ready." >&2
    else
        echo "[pddl-solver] GHCR pull failed. Falling back to local build..." >&2
    fi
fi
...
if [ -z "$IMAGE" ]; then
    if [ -z "$DOCKER_DIR" ]; then
        echo "[pddl-solver] No Dockerfile found for local build fallback." >&2
        exit 1
    fi
    ...
    if ! docker image inspect "$LOCAL_TAGGED" &>/dev/null; then
        echo "[pddl-solver] Building PDDL sandbox image locally (version ${VERSION_HASH})..." >&2
        echo "[pddl-solver] This compiles Fast Downward, Metric-FF, and VAL (~15 min on first build)." >&2
        ...
        if [ -n "$old_images" ]; then
            echo "[pddl-solver] Removing old image versions..." >&2
...
# -v workspace  : mount the user's working directory so planner can access PDDL files
